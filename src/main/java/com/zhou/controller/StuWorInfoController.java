...
public class StuWorInfoController {
    ...
    public String updateStudentFamInfo(StuWorInfo stuWorInfo, Model model, HttpServletRequest request) {
        ...
        try {
            ...
            if (i > 0) {
                model.addAttribute("id", stuWorInfo.getStuId());
    ...
    public String queryStudentById(@RequestParam(value = "pg", defaultValue = "1") Integer pg,@RequestParam(value = "queryStuWorInfoById", required = false) String queryStuWorInfoById, Model model, HttpServletRequest request) {
        PageHelper.startPage(pg, 5);
